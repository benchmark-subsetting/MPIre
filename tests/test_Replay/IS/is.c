...
/*  When NUM_PROCS approaching NUM_BUCKETS, it is highly possible
    ...
    //~{
      //~fprintf(stderr, "BEFORE Rank %d: send_count[%d] = %d, recv_count[%d] = %d\n", my_rank, i, send_count[i], i, recv_count[i]);
    //~}
/*  This is the redistribution section:  first find out how many keys
    ...
    //~for (i = 0; i < comm_size; i++)
    //~{
      //~fprintf(stderr, "AFTER Rank %d: send_count[%d] = %d, recv_count[%d] = %d\n", my_rank, i, send_count[i], i, recv_count[i]);
    //~}
/*  Determine the receive array displacements for the buckets */    
    ...
    //~if (my_rank == 0) {
      ...
      //~{
        //~fprintf(stderr, "BEFORE key_buff2[%d]=%d\n", i, key_buff2[i]);
      //~}
      //~fprintf(stderr, "IS : First element is %d\n", key_buff2[0]);
      //~fprintf(stderr, "IS : Second element is %d\n", key_buff2[1]);
    //~}
/*  Now send the keys to respective processors  */
    ...
    //~if (my_rank == 0) {
      ...
      //~for (i = 0; i < SIZE_OF_BUFFERS; i++)
      //~{
        //~fprintf(stderr, "AFTER key_buff2[%d]=%d\n", i, key_buff2[i]);
      //~}
      //~fprintf(stderr, "IS : First element is %d\n", key_buff2[0]);
      //~fprintf(stderr, "IS : Second element is %d\n", key_buff2[1]);
    //~}
...
/* shifted differently for different cases */
    //~fprintf(stderr, "TEST ARRAY SIZE = %d\n", TEST_ARRAY_SIZE);
    for( i=0; i<TEST_ARRAY_SIZE; i++ )
    {
        //~fprintf(stderr, "I = %d\n", i);
                ...
                case 'A':
                    if( i <= 2 )
                    {
                        if( key_rank != test_rank_array[i]+(iteration-1) ) {
                            ...
                            //~printf("key_rank = %ld sum = %ld : test_rank_array[%ld] = %ld + iteration = %ld \n", key_rank, test_rank_array[i]+iteration, i, test_rank_array[i], iteration); 
                            failed = 1;
                        }
                        else {
                            //~fprintf(stderr, "PASSED i = %d\n", i);
                            passed_verification++;
                        }
                    }
                    else
        ...
        //~else fprintf(stderr, "No verification for this i\n");
    }
